...
float4 _EmissionHsvm1,_EmissionHsvm2,_TransitionColor,_LineColor;
struct fakelitv2g{
...
struct Interpolators {
	...
	#endif
	float3 lightcolor : TEXCOORD7; //this is for the custom emission
	float4 colorparam : TEXCOORD8; // 这个用于边缘着色什么的解包
};
//custom function
...
fakelitv2g repackVertexData(float4 vertex,float3 normal,float4 tangent,float2 uv, float2 uv1,float4 colorparam){
...
Interpolators g2v2i(fakelitv2g g, float3 emission){ 
	//这里的意思是，我们的把geo作为vertex来用，然后再在geo里面把真正的vertex输出出去
	//因为geo实际上也是做了一个vertex的作用，只不过他需要拿其他的点进行输入
	//这个程序里面唯一改变的只有顶点和法线，其他切线uv这些变成cube也不是很好计算。。暂时先用原来的或者给一个数值，后面再看看怎么样更新uv
	...
	i.lightcolor = emission;
	i.colorparam = g.color; //这个用于解包。 color的计算在geo里面，不在顶点数据里面（没有什么意义）
	return i;
...
void geom(triangle litv2g IN[3], uint pid : SV_PrimitiveID, inout TriangleStream<Interpolators> outstream){ 
	...
	if(_Paramtest>center_ws.y){ //默认三角形
		...
		o1.uv1 = IN[1].uv1;
		o2.uv1 = IN[2].uv1;
		o0.color = float4(0.5,0.5,0,0);
		o1.color = float4(0.5,0.5,0,0);
		o2.color = float4(0.5,0.5,0,0);
		Interpolators realinput0;
		Interpolators realinput1;
		Interpolators realinput2;

		realinput0 = g2v2i(o0,float3(0,0,0));
		realinput1 = g2v2i(o1,float3(0,0,0));
		realinput2 = g2v2i(o2,float3(0,0,0));
	...
	if(distance(float3(0,_Paramtest,0),center_ws) > _effectordistance){ 
		...
		if(Hash(pid*100)<_Density){
			...
			float randomnumber2 = Hash(pid*100 + 5);
			...
			float3 customemission = lerp(0,_Emission,param);
			float3 nc = (float3(-1, 0, 0));//因为g2v2i程序中UnityObjectToWorldNormal，我们这里以中心点为原点产生的坐标系，对应的法线应该是模型空间下的，所以应该没有错？（待验证）
			outstream.Append( g2v2i( repackVertexData( lerpvertex(pc2,p0,lerpparam), nc, IN[0].tangent, IN[0].uv, IN[0].uv1,float4(float2(0,0),param,randomnumber2) ) ,customemission) ); //lerp the vertex, then the normal,the tangent, the uv, the uv1, //now we get a fakelitv2g
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc0,p2,lerpparam), nc, IN[2].tangent, IN[2].uv, IN[2].uv1,float4(float2(0,0),param,randomnumber2)) ,customemission) );
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc6,p0,lerpparam), nc, IN[0].tangent, IN[0].uv, IN[0].uv1,float4(float2(0,0),param,randomnumber2)) ,customemission) );
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc4,p2,lerpparam), nc, IN[2].tangent, IN[2].uv, IN[2].uv1,float4(float2(0,0),param,randomnumber2)) ,customemission) );
			outstream.RestartStrip();

			nc = (float3(1, 0, 0)); 
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc1,p2,lerpparam), nc, IN[2].tangent, IN[2].uv, IN[2].uv1,float4(float2(0,0),param,randomnumber2)) ,customemission) );
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc3,p1,lerpparam), nc, IN[1].tangent, IN[1].uv, IN[1].uv1,float4(float2(1,0),param,randomnumber2)) ,customemission) );
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc5,p2,lerpparam), nc, IN[2].tangent, IN[2].uv, IN[2].uv1,float4(float2(0,1),param,randomnumber2)) ,customemission) );
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc7,p1,lerpparam), nc, IN[1].tangent, IN[1].uv, IN[1].uv1,float4(float2(1,1),param,randomnumber2)) ,customemission) );
			outstream.RestartStrip();

			nc = (float3(0, -1, 0));

			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc0,p2,lerpparam), nc, IN[2].tangent, IN[2].uv, IN[2].uv1,float4(float2(0,0),param,randomnumber2)) ,customemission) );
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc1,p2,lerpparam), nc, IN[2].tangent, IN[2].uv, IN[2].uv1,float4(float2(1,0),param,randomnumber2)) ,customemission) );
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc4,p2,lerpparam), nc, IN[2].tangent, IN[2].uv, IN[2].uv1,float4(float2(0,1),param,randomnumber2)) ,customemission) );
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc5,p2,lerpparam), nc, IN[2].tangent, IN[2].uv, IN[2].uv1,float4(float2(1,1),param,randomnumber2)) ,customemission) );
			outstream.RestartStrip();

			nc = (float3(0, 1, 0));
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc3,p1,lerpparam), nc, IN[1].tangent, IN[1].uv, IN[1].uv1,float4(float2(0,0),param,randomnumber2)) ,customemission) );
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc2,p0,lerpparam), nc, IN[0].tangent, IN[0].uv, IN[0].uv1,float4(float2(1,0),param,randomnumber2)) ,customemission) );
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc7,p1,lerpparam), nc, IN[1].tangent, IN[1].uv, IN[1].uv1,float4(float2(0,1),param,randomnumber2)) ,customemission) );
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc6,p0,lerpparam), nc, IN[0].tangent, IN[0].uv, IN[0].uv1,float4(float2(1,1),param,randomnumber2)) ,customemission) );
			outstream.RestartStrip();

			nc = (float3(0, 0, -1));
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc1,p2,lerpparam), nc, IN[2].tangent, IN[2].uv, IN[2].uv1,float4(float2(0,0),param,randomnumber2)) ,customemission) );
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc0,p2,lerpparam), nc, IN[2].tangent, IN[2].uv, IN[2].uv1,float4(float2(1,0),param,randomnumber2)) ,customemission) );
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc3,p1,lerpparam), nc, IN[1].tangent, IN[1].uv, IN[1].uv1,float4(float2(0,1),param,randomnumber2)) ,customemission) );
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc2,p0,lerpparam), nc, IN[0].tangent, IN[0].uv, IN[0].uv1,float4(float2(1,1),param,randomnumber2)) ,customemission) );
			outstream.RestartStrip();

			nc = (float3(0, 0, 1));
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc4,p2,lerpparam), nc, IN[2].tangent, IN[2].uv, IN[2].uv1,float4(float2(0,0),param,randomnumber2)) ,customemission) );
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc5,p2,lerpparam), nc, IN[2].tangent, IN[2].uv, IN[2].uv1,float4(float2(1,0),param,randomnumber2)) ,customemission) );
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc6,p0,lerpparam), nc, IN[0].tangent, IN[0].uv, IN[0].uv1,float4(float2(0,1),param,randomnumber2)) ,customemission) );
			outstream.Append( g2v2i( repackVertexData(lerpvertex(pc7,p1,lerpparam), nc, IN[1].tangent, IN[1].uv, IN[1].uv1,float4(float2(1,1),param,randomnumber2)) ,customemission) );
			outstream.RestartStrip();
			}
		return;
...
float3 SelfEmission(Interpolators input)
{
    float2 bcc = input.colorparam.rg; 
    float em1 = saturate(input.colorparam.b); 
    float em2 = saturate(input.colorparam.b - 1);
    float rand = input.colorparam.a;
    float3 face = (lerp(_EmissionHsvm1.xyz, _EmissionHsvm2.xyz, rand)); 
    face *= lerp(_EmissionHsvm1.w, _EmissionHsvm2.w, rand); 
    face *= lerp(0.75, 1, smoothstep(0, 0.5, length(bcc - 0.5))); 
    float2 fw = fwidth(bcc); 
    half2 edge2 = min(smoothstep(0, fw*2, bcc), smoothstep(0, fw*2, 1 - bcc)); 
    half edge = 1 - min(edge2.x, edge2.y);
    return
        face * em1 + _TransitionColor * em2 * face + edge * _LineColor * em1;
...
FragmentOutput MyFragmentProgram (Interpolators i) {
	...
	color.rgb += SelfEmission(i);
	// color.rgb+=i.Eemission;
